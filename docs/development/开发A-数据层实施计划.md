...
> 状态：P0 已实现，MySQL 8.4.10 真实数据库联调通过
...
| DA-14 | 删除已售房屋 | 影响 0 行 |
| DA-15 | 空结果查询 | 返回空列表 |
| DA-16 | 同一房屋重复写入成交记录 | `uk_sale_house` 唯一约束拒绝第二次写入 |
...
| 日期 | 数据库 | 测试结果 | 数据清理 |
|------|--------|----------|----------|
| 2026-07-13 | MySQL Community Server 8.4.10 | 21 项通过，0 失败，0 跳过 | 三张业务表均为 0 条残留 |
...
- DAO 使用的列名与脚本完全一致；
- 中文楼盘名、地址和客户姓名写入后无乱码；
- `sale_record.house_id` 已增加唯一约束 `uk_sale_house`，落实一套房最多一条成交记录。
...
- [ ] 陈辉姓名已写入团队分工表，学号待填写；
- [ ] 分工冲突已经团队确认并同步到相关文档；
- [x] `DBConfig` 能通过环境变量或 properties 建立连接；
- [x] 四个模型类与数据字典一致；
- [x] `BuildingDao` 所有规定方法已实现；
- [x] `HouseDao` 所有规定方法已实现；
- [x] 所有 SQL 使用 `PreparedStatement`；
- [x] JDBC 资源均能正确关闭；
- [x] 购买事务所需的 Connection 重载已完成联调；
- [x] DAO 集成测试覆盖正常、边界和失败场景；
- [x] `mvn clean test` 和 `mvn compile` 通过；
- [x] JavaDoc 符合项目规范且作者信息真实；
- [x] 数据库设计、数据字典、SQL 和代码字段保持一致；
- [x] 联调问题和最终结果已记录。
...
> 陈辉负责系统数据层基础设施，完成数据库连接配置、楼盘与房屋实体建模，以及楼盘和房屋 DAO 的 JDBC 实现。通过 PreparedStatement、资源自动关闭、状态条件更新和数据库事务协作接口，保证数据访问的安全性与购买流程的数据一致性，并完成数据库脚本及 DAO 集成测试联调。

最终提交课程报告前，应补齐学号，并根据课程报告实际章节调整描述。
...
| 日期 | 参与人 | 评审结论 | 待办事项 |
|------|--------|----------|----------|
| 2026-07-13 | 陈辉 | 技术验收通过 | 补齐学号并由团队正式确认分工 |

### 15.2 变更记录

| 版本 | 日期 | 修改人 | 说明 |
|------|------|--------|------|
| v0.1 | 2026-07-11 | 陈辉 | 建立数据层主工程计划、接口契约和验收矩阵 |
| v0.2 | 2026-07-12 | 陈辉 | 完成 P0 数据层实现并对接业务层接口 |
| v0.3 | 2026-07-13 | 陈辉 | 完成真实 MySQL 联调、成交唯一约束及 SaleRecordDao 测试 |
