...
public class Test {
	...
	public static void main(String[] args) {
		...
		System.out.println("欢迎使用星云图书书店");
		test.accreditRole(user);//  授权角色
		BookBiz.makeData();//初始化图书信息
		if (user.getRole().getDescription()=="管理员")// 如果是库存管理员
			test.doStoreMgr(user);// 执行库管操作
		else if (user.getRole().getDescription()=="顾客")// 如果是顾客
